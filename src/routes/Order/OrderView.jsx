...
const prefixCls = 'contract-view-9999'
   ...
   const findLabel = (arr, value) => {
    ...
    <Main title={props.title} location={props.location}>
      <div className={prefixCls}>
        <SearchInput onSearch={onSearch} placeholder="订单编号/所诉公司"/>
        <div className="pick-group">
          ...
          >
          <div className="pick-group-item">{findLabel(companyList[0], company) || '订单状态'} <Icon type="down"/></div>
          </Picker>
          <Picker
            data={statusList}
            title="选择人员"
          ...
          >
          <div className="pick-group-item">{findLabel(statusList[0], status) || ''} <Icon type="down"/></div>
          </Picker>
        </div>
        <PagingListView
          style={{ height: '100%'}}
          ...
          renderRow={(rowData, sectionID, rowID) => {
            ...
            return (
             <div className="sale-card">
               ...
               <div className="btns">
                 <div onClick={() => {toPaymentRecord(rowData)}}>添加回款记录</div>
               </div>
             </div>
            )
          }}
